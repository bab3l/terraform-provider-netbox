...
var _ resource.ResourceWithIdentity = &CableResource{}
...
func (r *CableResource) IdentitySchema(ctx context.Context, req resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *CableResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	if parsed, ok := utils.ParseImportIdentityCustomFields(ctx, req.Identity, &resp.Diagnostics); ok {
		...
		if err != nil {
			resp.Diagnostics.AddError(
				"Invalid cable ID",
				fmt.Sprintf("Cable ID must be a number, got: %s", parsed.ID),
			)
			return
		}

		cable, httpResp, err := r.client.DcimAPI.DcimCablesRetrieve(ctx, id).Execute()
		defer utils.CloseResponseBody(httpResp)
		if err != nil {
			resp.Diagnostics.AddError(
				"Error importing cable",
				utils.FormatAPIError("read cable", err, httpResp),
			)
			return
		}
		...
		var data CableResourceModel
		data.Tags = utils.PopulateTagsSlugFromAPI(ctx, cable.HasTags(), cable.GetTags(), data.Tags)
		...
		resp.Diagnostics.Append(r.mapResponseToState(ctx, cable, &data)...)
		if resp.Diagnostics.HasError() {
			return
		}
		if parsed.HasCustomFields {
			data.CustomFields = utils.PopulateCustomFieldsFilteredToOwned(ctx, data.CustomFields, cable.GetCustomFields(), &resp.Diagnostics)
		} else {
			data.CustomFields = types.SetNull(utils.GetCustomFieldsAttributeType().ElemType)
		}
		if resp.Diagnostics.HasError() {
