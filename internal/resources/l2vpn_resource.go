...
func (r *L2VPNResource) IdentitySchema(ctx context.Context, req resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *L2VPNResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	if parsed, ok := utils.ParseImportIdentityCustomFields(ctx, req.Identity, &resp.Diagnostics); ok {
		...
		if err != nil {
			resp.Diagnostics.AddError("Invalid ID format", fmt.Sprintf("Could not parse L2VPN ID '%s': %s", parsed.ID, err.Error()))
			return
		}
		l2vpn, httpResp, err := r.client.VpnAPI.VpnL2vpnsRetrieve(ctx, idInt).Execute()
		defer utils.CloseResponseBody(httpResp)
		...
		if l2vpn.HasImportTargets() && len(l2vpn.GetImportTargets()) > 0 {
			...
			importSet, setDiags := types.SetValueFrom(ctx, types.StringType, targetIDs)
			resp.Diagnostics.Append(setDiags...)
			if resp.Diagnostics.HasError() {
				return
			}
			data.ImportTargets = importSet
		}
		if l2vpn.HasExportTargets() && len(l2vpn.GetExportTargets()) > 0 {
			...
			exportSet, setDiags := types.SetValueFrom(ctx, types.StringType, targetIDs)
			resp.Diagnostics.Append(setDiags...)
			if resp.Diagnostics.HasError() {
				return
			}
		...
		if parsed.HasCustomFields {
			data.CustomFields = utils.PopulateCustomFieldsFilteredToOwned(ctx, data.CustomFields, l2vpn.GetCustomFields(), &resp.Diagnostics)
		} else {
			data.CustomFields = types.SetNull(utils.GetCustomFieldsAttributeType().ElemType)
		}
		if resp.Diagnostics.HasError() {
