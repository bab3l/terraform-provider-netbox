...
func TestAccInventoryItemDataSource_byNameAndDevice(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccInventoryItemDataSourceConfigByNameAndDevice(siteName, siteSlug, roleName, roleSlug, mfgName, mfgSlug, deviceTypeName, deviceTypeSlug, deviceName, itemName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.netbox_inventory_item.test", "name", itemName),
					resource.TestCheckResourceAttrSet("data.netbox_inventory_item.test", "device_id"),
				),
...
func testAccInventoryItemDataSourceConfigByNameAndDevice(siteName, siteSlug, roleName, roleSlug, mfgName, mfgSlug, deviceTypeName, deviceTypeSlug, deviceName, itemName string) string {
	return fmt.Sprintf(`
resource "netbox_site" "test" {
  name = "%s"
  slug = "%s"
...
data "netbox_inventory_item" "test" {
  name      = netbox_inventory_item.test.name
