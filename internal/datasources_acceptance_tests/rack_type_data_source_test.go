...
func TestAccRackTypeDataSource_bySlug(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRackTypeDataSourceConfigBySlug(mfrName, mfrSlug, model, slug),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.netbox_rack_type.test", "model", model),
					resource.TestCheckResourceAttr("data.netbox_rack_type.test", "slug", slug),
				),
...
func TestAccRackTypeDataSource_byModel(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRackTypeDataSourceConfigByModel(mfrName, mfrSlug, model, slug),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.netbox_rack_type.test", "model", model),
					resource.TestCheckResourceAttr("data.netbox_rack_type.test", "slug", slug),
				),
...
func testAccRackTypeDataSourceConfigBySlug(mfrName, mfrSlug, model, slug string) string {
	return fmt.Sprintf(`
resource "netbox_manufacturer" "test" {
  name = "%s"
  slug = "%s"
...
func testAccRackTypeDataSourceConfigByModel(mfrName, mfrSlug, model, slug string) string {
	return fmt.Sprintf(`
resource "netbox_manufacturer" "test" {
  name = "%s"
  slug = "%s"
