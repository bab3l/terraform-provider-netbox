...
func TestRIRDataSource(t *testing.T) {
	d := datasources.NewRIRDataSource()
	if d == nil {
		t.Fatal("RIR data source should not be nil")
	}
}
...
func TestRIRDataSourceSchema(t *testing.T) {
	...
	expectedAttrs := []string{"id", "name", "slug", "is_private", "description", "tags"}
