...
func TestLocationDataSource(t *testing.T) {
	d := NewLocationDataSource()
	if d == nil {
		t.Fatal("Location data source should not be nil")
	}
}
...
func TestLocationDataSourceSchema(t *testing.T) {
	...
	expectedAttrs := []string{"id", "name", "slug", "site", "parent", "status", "description", "tags", "custom_fields"}
