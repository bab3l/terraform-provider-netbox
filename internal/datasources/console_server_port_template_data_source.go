...
var _ datasource.DataSource = &ConsoleServerPortTemplateDataSource{}
...
// ConsoleServerPortTemplateDataSourceModel describes the data source data model.
...
func (d *ConsoleServerPortTemplateDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "Retrieves information about a console server port template in NetBox.",
	...
	case !data.Name.IsNull() && !data.Name.IsUnknown():
		...
		if err != nil {
			...
			resp.Diagnostics.AddError(
				...
				utils.FormatAPIError(fmt.Sprintf("read console server port template by name %s", name), err, httpResp),
			)
		...
		if count == 0 {
			...
			resp.Diagnostics.AddError(
				...
				fmt.Sprintf("No console server port template found with name: %s", name),
			)
		...
		if count > 1 {
			...
			resp.Diagnostics.AddError(
				...
				fmt.Sprintf("Found %d console server port templates with name %s. Please specify device_type or module_type to narrow results, or use ID.", count, name),
			)
